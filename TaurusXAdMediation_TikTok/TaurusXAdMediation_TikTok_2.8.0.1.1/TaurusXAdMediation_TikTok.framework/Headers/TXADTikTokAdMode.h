...
typedef NS_ENUM(NSUInteger, TXADTiktokNativeMode) {
    TXAD_TIKTOK_NATIVE_BANNER = 0, // 自渲染 Banner（穿山甲已弃用）
    // TXAD_TIKTOK_NATIVE_EXPRESS = 1, // 个性化模版原生（无此类型）
    TXAD_TIKTOK_NATIVE_INTERSTITIAL = 2, // 自渲染插屏（穿山甲已弃用）
    TXAD_TIKTOK_NATIVE_FEEDLIST = 3 // 自渲染信息流（支持海外使用 Native Api 来访问信息流）
};
